...
function LocestaLogo({ size = 56 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" fill="none">
      <rect width="100" height="100" rx="22" fill="#0f1923"/>
      <path d="M50 8 C30 8 14 24 14 44 C14 64 50 92 50 92 C50 92 86 64 86 44 C86 24 70 8 50 8Z" fill="url(#pinReg2)"/>
      ...
      <defs>
        <linearGradient id="pinReg2" x1="14" y1="8" x2="86" y2="92" gradientUnits="userSpaceOnUse">
...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name.trim()) { setError("Jaza jina lako kamili"); return; }
    if (!form.email.trim()) { setError("Jaza email yako"); return; }
    if (!form.password) { setError("Jaza password"); return; }
    ...
    try {
      ...
      if (msg?.toLowerCase().includes("exist") || msg?.toLowerCase().includes("already")) {
        ...
        setError("Email si sahihi — angalia tena");
      } else {
  ...
  const handleGoogle = async (credentialResponse: { credential?: string }) => {
    try {
      ...
      navigate("/");
    } catch { setError("Google login imeshindwa — jaribu tena"); }
  };
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(145deg, #0f1923 0%, #1a2a3a 100%)", display: "flex", alignItems: "stretch" }}>
      ...
      <div className="login-left-panel" style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "3rem 2rem", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, textAlign: "center", maxWidth: "360px" }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "1rem", marginBottom: "2.5rem" }}>
            <LocestaLogo size={80} />
            <div>
              <div style={{ fontSize: "2.4rem", fontWeight: 800, letterSpacing: "0.02em", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="login-right-panel" style={{ width: "440px", background: cardBg, display: "flex", alignItems: "center", justifyContent: "center", padding: "3rem 2.5rem" }}>
        <div style={{ width: "100%", maxWidth: "340px" }}>
          ...
          {error && (
            <div style={{ background: "#fef2f2", border: "1px solid #fca5a5", color: "#dc2626", padding: "0.875rem 1rem", borderRadius: "10px", fontSize: "0.875rem", marginBottom: "1.25rem", display: "flex", alignItems: "flex-start", gap: "8px" }}>
