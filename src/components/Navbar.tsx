  ...
  return (
    <>
      <nav style={{
        ...
        background: dark ? "rgba(8,14,22,0.97)" : "rgba(13,27,42,0.97)",
        ...
        <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 1.25rem", height: "68px", display: "flex", alignItems: "center", gap: "0.75rem" }}>
          ...
          <Link to="/" style={{ display: "flex", alignItems: "center", marginRight: "auto", textDecoration: "none", flexShrink: 0 }}>
            <LOKESTALogo size={40} showText={true} />
          </Link>
        ...
        {menuOpen && (
          <div style={{ background: "#080f16", borderTop: "1px solid rgba(249,115,22,0.12)", padding: "0.75rem 1.25rem 1.25rem", display: "flex", flexDirection: "column", gap: "0.2rem" }}>
            {/* Mobile logo in menu */}
            <div style={{ padding: "0.5rem 0 0.75rem", display: "flex", alignItems: "center", gap: "10px" }}>
              <LOKESTALogo size={32} showText={true} />
            </div>
            <div style={{ height: "1px", background: "rgba(255,255,255,0.07)", marginBottom: "0.25rem" }} />

            {navLinks.map(({ to, label }) => (
              <Link key={to} to={to} style={{ padding: "0.875rem 1rem", borderRadius: "10px", fontSize: "0.95rem", fontWeight: 500, textDecoration: "none", color: isActive(to) ? "#f97316" : "rgba(255,255,255,0.75)", background: isActive(to) ? "rgba(249,115,22,0.1)" : "transparent" }} onClick={() => setMenuOpen(false)}>
                {label}
              </Link>
            ))}
