  ...
  return (
    <div style={{ background: bg, transition: "background 0.3s" }}>
      ...
      <div style={{ position: "relative", height: "100vh", minHeight: "580px", maxHeight: "900px", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to bottom, rgba(15,25,35,0.7) 0%, rgba(15,25,35,0.35) 50%, rgba(15,25,35,0.9) 100%)" }} />
        ...
        <div style={{ position: "relative", zIndex: 1, height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "2rem 1.25rem", textAlign: "center" }}>
          ...
          <div style={{ display: "flex", width: "100%", maxWidth: "540px", borderRadius: "14px", overflow: "hidden", boxShadow: "0 8px 40px rgba(0,0,0,0.3)", background: "#fff" }} className="hero-search">
            <div style={{ display: "flex", alignItems: "center", flex: 1, padding: "0 1.25rem", gap: "10px" }} className="hero-search-input-wrap">
            ...
            <button onClick={handleSearch} className="hero-search-btn" style={{ background: "linear-gradient(135deg, #f97316, #fbbf24)", color: "#fff", border: "none", padding: "0 1.5rem", fontWeight: 700, fontSize: "0.9rem", cursor: "pointer", fontFamily: "'DM Sans', sans-serif", display: "flex", alignItems: "center", gap: "6px", whiteSpace: "nowrap" }}>
              {t("home.searchBtn")}
            </button>
          </div>
          ...
          <div style={{ display: "flex", gap: "0.5rem", marginTop: "1.25rem", flexWrap: "wrap", justifyContent: "center" }}>
            ...
            {["Arusha", "Dar es Salaam", "Zanzibar", "Moshi"].map((c) => (
              <button key={c} onClick={() => navigate(`/rooms?city=${c}`)} style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", color: "#fff", padding: "4px 14px", borderRadius: "20px", fontSize: "0.78rem", cursor: "pointer", fontFamily: "'DM Sans', sans-serif", backdropFilter: "blur(8px)" }}>
                {c}
              </button>
            ))}
          </div>
      ...
      <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "4rem 1.25rem 2rem" }}>
        ...
        {rooms && rooms.length > 0 ? (
              ...
              <Link key={room.id} to={`/property/${room.id}`} style={{ textDecoration: "none" }}>
                ...
                >
                  <div style={{ position: "relative", height: "175px" }}>
                    ...
                    <div style={{ position: "absolute", top: "10px", right: "10px", background: "rgba(15,25,35,0.85)", color: "#f97316", padding: "4px 10px", borderRadius: "20px", fontSize: "0.78rem", fontWeight: 700 }}>
          ...
          <div style={{ textAlign: "center", padding: "3rem", background: cardBg, borderRadius: "16px", border: `1px solid ${borderColor}` }}>
            ...
            <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: "1.3rem", color: textPrimary, margin: "1rem 0 0.5rem" }}>Vyumba vitaonekana hapa</h3>
            <p style={{ color: textSecondary, fontSize: "0.875rem", marginBottom: "1.5rem" }}>Anza kuongeza vyumba kwenye LOKESTA</p>
            <Link to="/add-property" style={{ background: "linear-gradient(135deg, #f97316, #fbbf24)", color: "#fff", padding: "0.75rem 1.75rem", borderRadius: "10px", fontWeight: 700, fontSize: "0.875rem" }}>+ Ongeza Chumba</Link>
          </div>
        )}
      </div>
      ...
      <div style={{ background: "#0f1923", padding: "4rem 1.25rem" }}>
        <div style={{ maxWidth: "1000px", margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: "2.5rem" }}>
            <h2 className="section-title" style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(1.5rem, 3vw, 2rem)", color: "#fff", marginBottom: "0.4rem" }}>Jinsi LOKESTA Inavyofanya Kazi</h2>
            <p style={{ color: "rgba(255,255,255,0.45)", fontSize: "0.85rem" }}>Hatua 4 rahisi kupata chumba chako</p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(190px, 1fr))", gap: "1.25rem" }} className="steps-grid">
            {[
              ...
              <div key={s.num} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(249,115,22,0.15)", borderRadius: "14px", padding: "1.5rem 1.25rem", textAlign: "center" }}>
                <div style={{ fontFamily: "'Playfair Display', serif", fontSize: "0.72rem", color: "#f97316", marginBottom: "0.75rem", letterSpacing: "0.1em" }}>{s.num}</div>
                <div style={{ fontSize: "1.75rem", marginBottom: "0.75rem" }}>{s.icon}</div>
                <h3 style={{ fontFamily: "'Playfair Display', serif", color: "#fff", fontSize: "1rem", marginBottom: "0.4rem" }}>{s.title}</h3>
                <p style={{ color: "rgba(255,255,255,0.45)", fontSize: "0.8rem", lineHeight: 1.5 }}>{s.desc}</p>
              </div>
            ))}
          </div>
        </div>
